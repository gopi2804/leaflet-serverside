(function (f) {
    ...
    return (function e(t, n, r) {
        ...
        1: [function (require, module, exports) {
            ...
            let cacheBusterDate = +new Date();

            // leaflet-image
            module.exports = function leafletImage(map, callback) {

                let hasMapbox = !!L.mapbox;
                ...
                if (map._pathRoot) {
                    ...
                    let firstCanvas = map._panes.overlayPane.getElementsByTagName('canvas').item(0);
                    if (firstCanvas) {
                ...
                function handleTileLayer(layer, callback) {
                    ...
                    let isCanvasLayer = (L.TileLayer.Canvas && layer instanceof L.TileLayer.Canvas),
                    ...
                    tiles.forEach(function (tilePoint) {
                        ...
                        if (tilePoint.y >= 0) {
                            if (isCanvasLayer) {
                                let tile = layer._tiles[tilePoint.x + ':' + tilePoint.y];
                                tileQueue.defer(canvasTile, tile, tilePos, tileSize);
                            } else {
                                let url = addCacheString(layer.getTileUrl(tilePoint));
                ...
                function handlePathRoot(root, callback) {
                    ...
                    canvas.height = dimensions.y;
                    let ctx = canvas.getContext('2d');
                    let pos = L.DomUtil.getPosition(root).subtract(bounds.min).add(origin);
                    try {
                ...
                function handleMarkerLayer(marker, callback) {
                    ...
                    let x = Math.round(pos.x - size[0] + anchor.x),
                ...
                function isDataURL(url) {
                    let dataURLRegex = /^\s*data:([a-z]+\/[a-z]+(;[a-z\-]+\=[a-z\-]+)?)?(;base64)?,[a-z0-9\!\$\&\'\,\(\)\*\+\,\;\=\-\.\_\~\:\@\/\?\%\s]*\s*$/i;
